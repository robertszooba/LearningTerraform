...
// Firewall rule for the health check added manually
// According to: https://cloud.google.com/load-balancing/docs/health-checks#gcloud_7
...
module "instance_template" {
  ...
  tags = ["allow-network-lb-health-checks"]
  machine_type = "e2-standard-2"
  startup_script = <<-EOF
  #!/bin/bash
  sudo snap install docker
